...
import xyz.wongs.drunkard.war.core.domain.SysRoleDept;
import xyz.wongs.drunkard.war.core.domain.SysRoleMenu;
import xyz.wongs.drunkard.war.core.domain.SysUserRole;
import xyz.wongs.drunkard.war.core.mapper.SysRoleDeptMapper;
import xyz.wongs.drunkard.war.core.mapper.SysRoleMapper;
...
public class SysRoleServiceImpl implements ISysRoleService {
     ...
     */
    @Override
    public int deleteRoleByIds(String ids) throws DrunkardException {
        Long[] roleIds = Convert.toLongArray(ids);
        for (Long roleId : roleIds) {
            checkRoleAllowed(new SysRole(roleId));
            SysRole role = selectRoleById(roleId);
            if (countUserRoleByRoleId(roleId) > 0) {
                throw new DrunkardException(String.format("%1$s已分配,不能删除", role.getRoleName()));
            }
        }
    ...
    public void checkRoleAllowed(SysRole role) {
        if (StringUtils.isNotNull(role.getId()) && role.isAdmin()) {
            throw new DrunkardException("不允许操作超级管理员角色");
        }
    }
