...
import xyz.wongs.drunkard.war.core.mapper.SysPostMapper;
...
public class SysPostServiceImpl implements ISysPostService {
     ...
     */
    @Override
    public int deletePostByIds(String ids) throws DrunkardException {
        Long[] postIds = Convert.toLongArray(ids);
        for (Long postId : postIds) {
            SysPost post = selectPostById(postId);
            if (countUserPostById(postId) > 0) {
                throw new DrunkardException(String.format("%1$s已分配,不能删除", post.getPostName()));
            }
        }
