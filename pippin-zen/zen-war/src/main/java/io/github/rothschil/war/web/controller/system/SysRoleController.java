...
public class SysRoleController extends BaseController {
    ...
    @ApplicationLog(title = "角色管理", businessType = BusinessType.EXPORT)
    ...
    @ApplicationLog(title = "角色管理", businessType = BusinessType.INSERT)
    @PostMapping("/add")
    @ResponseBody
    ...
    @ApplicationLog(title = "角色管理", businessType = BusinessType.UPDATE)
    @PostMapping("/edit")
    @ResponseBody
    ...
    public AjaxResult authDataScopeSave(SysRole role) {
        ...
        if (roleService.authDataScope(role) > 0) {
            ShiroUtils.setSysUser(userService.selectUserById(ShiroUtils.getSysUser().getId()));
            return success();
        }
        return error();
    }
    ...
    @ApplicationLog(title = "角色管理", businessType = BusinessType.DELETE)
    @PostMapping("/remove")
    @ResponseBody
     ...
     * 取消授权
     */
    @ApplicationLog(title = "角色管理", businessType = BusinessType.GRANT)
